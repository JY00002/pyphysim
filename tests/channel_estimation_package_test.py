...
class ChannelEstimationFunctionsTest(unittest.TestCase):
    ...
    def test_compute_mmse_estimation(self):
        ...
        # xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
        generate_channel_with_correlation(alpha, 3, Nt, np.ones((3, 3)))
        ...
        expected_h_mmse = np.concatenate([
            compute_mmse_estimation(Y1, s, noise_power, C)[np.newaxis],
            compute_mmse_estimation(Y2, s, noise_power, C)[np.newaxis]
        ])
        h_mmse = compute_mmse_estimation(Y_concat, s, noise_power, C)
        np.testing.assert_array_almost_equal(h_mmse, expected_h_mmse)
        ...
        Y_concat = np.concatenate([Y1[np.newaxis], Y2[np.newaxis]])
        expected_h_mmse = np.concatenate([
            compute_mmse_estimation(Y1, s[0], noise_power, C)[np.newaxis],
            compute_mmse_estimation(Y2, s[1], noise_power, C)[np.newaxis]
        ])
